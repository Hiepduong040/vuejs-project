<template>
  <div class="imageProduct_form">
    <div class="image_product_title">Hình ảnh sản phẩm</div>
      ...
      <div class="img_box" v-if="image && image.length > 0">
        <div class="flex relative" v-for="img in image" :key="img">
          ...
          <div
            ...
            class="absolute top-0 right-0   shadow cursor-pointer"
          >
...
const handleImageChange = async (event) => {
  ...
  const newUrls = []; // Mảng để lưu trữ URL mới
  ...
  if (files && files.length > 0) {
    for (const file of files) {
      ...
      try {
        ...
        newUrls.push(url); // Thêm URL mới vào mảng
      } catch (error) {
        console.log("Lỗi tải lên:", error);
      }
    }
    // Gửi mảng URL mới kết hợp với các URL đã có
    emits("handleImange", [...props.image, ...newUrls]);
  }
};
// Hàm xóa hình ảnh
const removeImage = (url) => {
  const updatedImages = props.image.filter((img) => img !== url);
  emits("handleImange", updatedImages); // Cập nhật danh sách ảnh
};
</script>
  <style scoped>
.main_content {
