...
const handleSubmit = async () => {
  if (validateForm()) {
    ...
    console.log(errors.value); // Optional: Log errors for debugging
  }
};
</script>
...
<!-- 
    ...
    const showModal = ref(false);
    const errors = ref({});
    const existingProducts = ref([]); // Store existing products
    ...
    const fetchProducts = async () => {
      existingProducts.value = await store.dispatch("fetchProducts"); // Get existing products
    };
    ...
    const handleImage = (link) => {
  editedProduct.value.image.push(link); // Assuming you're adding the image link to an array
};
  ...
  // Kiểm tra nếu existingProducts đã được khởi tạo và có dữ liệu
  if (existingProducts.value && existingProducts.value.length > 0) {
    // Kiểm tra trùng tên sản phẩm
    const isDuplicate = existingProducts.value.some(
      (existingProduct) =>
    ...
    if (isDuplicate) {
      errors.value.product_name = "Tên sản phẩm đã tồn tại! Vui lòng nhập tên sản phẩm khác.";
      isValid = false;
    }
  }
  ...
  return isValid;
    ...
    const handleSubmit = async () => {
      if (validateForm()) {
        ...
        showModal.value = false; // Close modal after adding product
        product.value = {
          product_name: "",
          description: "",
          unit_price: "",
